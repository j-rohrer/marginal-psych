...
dat <- transform(dat,
    ...
    gpa_gap = factor(ifelse(diff_gpa > median(diff_gpa, na.rm = TRUE), "High", "Low"))
)
...
#### Is the effect of sharing a desk stronger for students on girls-only desks, relative to the effect of sharing a desk for students on mixed-gender desks?
...
#### What is the average treatment effect of sharing a desk for students with high and low differences in GPA?

```{r}
cate <- avg_comparisons(mod,
    variables = "deskmate",
    by = "gpa_gap"
)
cate
...
When the gap in GPA is high, sharing a desk increases the probability of forming a friendship by `{r} sprintf("%.1f", cate$estimate[3] * 100)` percentage points. When the gap in GPA is low, sharing a desk increases the probability of forming a friendship by `{r} sprintf("%.1f", cate$estimate[2] * 100)` percentage points.

However, the difference these two estimates is *not* statistically significant at the 5% level.
