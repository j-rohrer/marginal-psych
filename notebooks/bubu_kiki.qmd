...
#### What is the average expected reaction time for Congruent and Incongruent stimuli?^[Note that the `by` argument accepts any variable from the dataset, even if that variable is not a predictor in the model.]

```{r}
avg_predictions(mod, by = "Congruent")
```

Reaction time appears shorter (faster) for congruent stimuli.
...
#### What is the average treatment effect of "Word" on reaction time?

```{r}
cmp <- avg_comparisons(mod, variables = "Word")
cmp
```

On average, moving from "Bubu" to "Kiki" increases reaction time by `{r} sprintf("%.3f", cmp$estimate)` seconds.

#### What is the average treatment effect of "Shape" on reaction time?

```{r}
cmp <- avg_comparisons(mod, variables = "Shape")
cmp
```

On average, moving from "Rounded" to "Spikey" reduces reaction time by `{r} sprintf("%.3f", abs(cmp$estimate))` seconds.

#### Do the estimated effect sizes of "Word" and "Shape" vary across participants, depending on the values of the other predictors in the model?

This plot shows the distribution of estimated counterfactual comparisons, for the two predictors of interest, across all the participants in the study.
...
There appears to be considerable heterogeneity in the effects of `Word` and `Shape` across different combinations of predictors. For some participants, moving from Bubu to Kiki is expected to increase reaction time by about 0.8 seconds, whereas for others the estimated effect is negative. Similarly, there is considerable heterogeneity in the estimated individual-level association between `Shape` and reaction time.

#### What is the conditional average treatment effect of "Word", for different values of "Shape"?

In other words, what is the effect of `Word` at different values of the `Shape` variable?
...
#### Is the conditional average treatment effect of "Word" larger when the shape is Rounded than when the shape is Spikey?
...
Yes. Although switching from "Bubu" to "Kiki" always increases reaction time, regardless of the shape, this positive effect is more pronounced when the shape is "Rounded". The difference between `Word` effect sizes under the two `Shape` conditions is `{r} sprintf("%.3f", cmp$estimate)` seconds.
