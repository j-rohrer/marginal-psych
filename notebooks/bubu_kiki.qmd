...
ggplot(dat = congruence_effect, aes(x = estimate)) +
    ...
    labs(x = "Congruence effect (in seconds)", y = "Density") +
...
alternative_pred <- avg_predictions(alternative, by = c("Spikes", "Kiki"))

ggplot(alternative_pred, aes(x = as.factor(Kiki), y = estimate, group = Spikes, shape = as.factor(Spikes), color = as.factor(Spikes))) +
