...
So far, we have conducted counterfactual comparisons across deskmate, and taken averages for different gender_combinations. 
When calculating the counterfactual comparison across deskmate, we hold all other variables in the model at their observed values and only vary deskmate to isolate the effects. 
However, when taking the averages for different gender_combinations, we do not hold anything at any particular value—we simply aggregate the comparisons for each of the different groups, which may also vary on other variables.

Thus, what we have calculated so far is the unconditional association between gender_combination and the effect of being seated next to each other. 
If, instead, we evaluated moderation by looking at model coefficients, we would look at conditional associations—because the coefficients always express associations conditional on all other variables in the model. 
Thus, when evaluating moderation, the default approach in the framework we propose  differs from the default approach when interpreting coefficients. 
In our approach, the association between the effects and the moderator is not “controlled for” all the other variables in the model. 
When instead relying on regression coefficients, the association between the effects and the moderator is controlled for all other variables in the model.

This naturally leads to the question whether we do want to control for other variables when evaluating moderation. 
First of all, this depends on the precise estimand. 
The term “moderation” can be used to refer to (1) merely the association between some moderator variable and the effects of some treatment or (2) a causal interaction in which intervening on the moderator would causally affect the effects of the treatment. 
If we are interested just in the former, no additional control would be necessary. 
If we are interested in the latter, additional control may be necessary to isolate the causal interaction. 
However, whether any given variable should be controlled for depends on the underlying causal net; inappropriate control variables may actually increase bias (Rohrer, 2018; Wysocki et al., 2022).

What applies to our scenario? 
Let’s say we are interested in whether gender_combination causally interacts with deskmate. 
So, in principle, additional control may be necessary. 
...
For gender-mismatched dyads, we may observe larger differences in GPA because gender affects GPA (Gender combination → GPA difference → Effect of being seated next to each other; Figure 4). 
...
Thus, to evaluate the causal interaction between GPA_difference and deskmate, we do want to control for gender_combination. If differences in grades affect the effect of the intervention, then we should be able to observe an association between differences in grades and the effects of the intervention among dyads of the same type (e.g., among all female dyads).
