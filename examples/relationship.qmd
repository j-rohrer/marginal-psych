...
## Model fitting and interpretation with `marginaleffects`: Basis splines
...
## Model fitting and interpretation with `marginaleffects`: Smoothing splines

In contrast to basis splines, smoothing splines do not force us to set the degrees of freedom.
Instead, smoothing splines estimate the smoothness parameter directly from the data

```{r}

# Make a copy of the data-set because
# we need to change some variable types for this to work
dat2 <- dat
dat2$gender <- factor(dat$gender, ordered = TRUE)
dat2$partner <- factor(dat$partner, ordered = TRUE)
dat2$friendship_importance_factor <- factor(dat2$friendship_importance, ordered = TRUE)

# Implementation with the help of mgcv::gam(), generalized additive models
mod2 <-  gam(friendship_importance ~ s(age) + gender + partner +
               s(age, by = gender) + partner:gender + s(age, by = partner), data = dat2)

# Predictions for a 20 year old single
predictions(mod2, newdata = datagrid(age = 20, gender = "male", partner = 0))

# Slope of age for a 20 year old single
slopes(mod2, variables = "age", newdata = datagrid(age = 20, gender = "male", partner = 0))

# Predictions for everybody in the data
predictions(mod2)

# Main result: Difference in friendship importance with and without partner, holding constant age and gender
avg_comparisons(mod2, variables = "partner")

# Generate separate estimates of the difference, by gender
avg_comparisons(mod2, variables = "partner", by = "gender")

# Compare these gender-specific estimates
avg_comparisons(mod2, variables = "partner", by = "gender", hypothesis = "b2 - b1 = 0")

# For the sake of completeness, we may also generate age-specific estimates

# Note that the wiggliness of this line will depend on how we modeled age
comp2 <- avg_comparisons(mod2, variables = "partner", by = "age")

ggplot(comp2, aes(x = age, y = estimate, ymin = conf.low, ymax = conf.high)) +
  geom_line() +
  geom_ribbon(alpha = .1) +
  geom_hline(yintercept = 0) +
  ylab("Association partner and friendship importance")
...
This results in a distribution for which the assumptions of linear regression may be considered unrealistic. 

So, let’s run an ordinal regression to see whether conclusions change. 


Here, we are going to fit a cumulative ordinal model with a probit link using the brms package [(Bürkner, 2018)](https://cran.r-project.org/web/packages/brms/index.html){target="_blank"}. 
...
saveRDS(mod_ord, here("fits/relationship_ordinal_brm.rds"))

```

```{r}
#| include: false
mod_ord <- readRDS(here("fits/relationship_ordinal_brm.rds"))
```
...
```{r}
# This is how, in principle, one would conduct the ordinal robustness check in a Frequentist ordinal model
...
# Likely due to an upstream bug in clm()
